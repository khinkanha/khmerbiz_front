import { defineComponent, ref, computed, resolveComponent, unref, withCtx, createVNode, withModifiers, toDisplayString, readonly, useSSRContext } from 'vue';
import { ssrRenderAttrs, ssrRenderStyle, ssrRenderAttr, ssrInterpolate, ssrRenderComponent } from 'vue/server-renderer';
import { f as defineStore, b as useRuntimeConfig, m as useState } from './server.mjs';
...
const useUpload = () => {
  ...
  const uploadFile = async (file, folder = "uploads", onProgress) => {
    const upload = {
      file,
      progress: 0,
      status: "pending"
...
const useMediaStore = defineStore("media", () => {
  ...
  const uploadMedia = async (file, folder = "uploads", onProgress) => {
  ...
  const searchMedia = async (query) => {
    try {
      const response = await api.get(`/media/search?q=${encodeURIComponent(query)}`);
      if (response.success && response.data) {
...
const _sfc_main = /* @__PURE__ */ defineComponent({
  ...
  setup(__props) {
    ...
    return (_ctx, _push, _parent, _attrs) => {
      ...
      _push(`<div${ssrRenderAttrs(_attrs)}><form style="${ssrRenderStyle({ "margin-bottom": "10px" })}"><input type="text"${ssrRenderAttr("value", unref(searchQuery))} class="form-control"${ssrRenderAttr("placeholder", _ctx.$t("contentManager.search"))}></form><div class="text-right" style="${ssrRenderStyle({ "margin-bottom": "10px" })}"><a href="#" class="btn btn-info"><i class="fa fa-plus"></i></a></div>`);
      if (unref(showUpload)) {
...
//# sourceMappingURL=index-DjViCL3i.mjs.map

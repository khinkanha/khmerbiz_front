...
import { g as useI18n, b as useRuntimeConfig } from './server.mjs';
...
const _sfc_main = /* @__PURE__ */ defineComponent({
  ...
  setup(__props) {
    const config = useRuntimeConfig();
    config.public.recaptchaEnabled;
    useAuthStore();
    const { t } = useI18n();
    useRecaptcha();
    const form = ref({
    ...
    return (_ctx, _push, _parent, _attrs) => {
      ...
      if (("useRuntimeConfig" in _ctx ? _ctx.useRuntimeConfig : unref(useRuntimeConfig))().public.recaptchaEnabled) {
        _push(`<div id="recaptcha-login"></div>`);
      } else {
        _push(`<!---->`);
      }
      _push(`</div><button type="submit" class="btn btn-danger"${ssrIncludeBooleanAttr(unref(loading)) ? " disabled" : ""}><i class="fa fa-sign-in-alt"></i> ${ssrInterpolate(_ctx.$t("auth.login"))}</button> \xA0 `);
...
//# sourceMappingURL=login-CSwv68Fz.mjs.map

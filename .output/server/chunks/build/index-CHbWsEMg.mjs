...
import { ssrRenderAttrs, ssrRenderAttr, ssrIncludeBooleanAttr, ssrRenderStyle, ssrLooseContain, ssrInterpolate } from 'vue/server-renderer';
...
const _sfc_main = /* @__PURE__ */ defineComponent({
  ...
  setup(__props) {
    ...
    return (_ctx, _push, _parent, _attrs) => {
      _push(`<div${ssrRenderAttrs(mergeProps({ class: "container" }, _attrs))}><h1>Make an announcement</h1><form><div class="form-group"><label>Subject</label><input type="text"${ssrRenderAttr("value", unref(form).subject)} class="form-control" placeholder="Subject"></div><div class="row"><div class="col-md-6"><div class="form-group"><label>To Email Address</label><input type="email"${ssrRenderAttr("value", unref(form).to_email)} class="form-control" placeholder="recipient@example.com"${ssrIncludeBooleanAttr(unref(form).send_to_all) ? " disabled" : ""}></div></div><div class="col-md-6"><div class="form-group" style="${ssrRenderStyle({ "margin-top": "25px" })}"><label><input type="checkbox"${ssrIncludeBooleanAttr(Array.isArray(unref(form).send_to_all) ? ssrLooseContain(unref(form).send_to_all, null) : unref(form).send_to_all) ? " checked" : ""}> Send to all </label></div></div></div><div class="form-group"><label>Message</label><textarea class="form-control" rows="10" placeholder="Write your announcement message...">${ssrInterpolate(unref(form).message)}</textarea></div>`);
      ...
      _push(`<button type="submit" class="btn btn-danger"${ssrIncludeBooleanAttr(unref(sending)) ? " disabled" : ""}>Send Email</button></form></div>`);
    };
  }
});
...
_sfc_main.setup = (props, ctx) => {
  ...
  (ssrContext.modules || (ssrContext.modules = /* @__PURE__ */ new Set())).add("pages/admin/super/announcing/index.vue");
  return _sfc_setup ? _sfc_setup(props, ctx) : void 0;
};

export { _sfc_main as default };
//# sourceMappingURL=index-CHbWsEMg.mjs.map

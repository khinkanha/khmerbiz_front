...
import { q as useToast, f as defineStore } from './server.mjs';
...
const useUserStore = defineStore("user", () => {
  ...
  const fetchUsers = async (page = 1) => {
    try {
      const response = await api.get(`/users?page=${page}&limit=${pagination.value.limit}`);
...
const _sfc_main = /* @__PURE__ */ defineComponent({
  ...
  setup(__props) {
    ...
    const onPageChange = (event) => {
      loading.value = true;
      userStore.fetchUsers(event.page + 1).finally(() => {
        loading.value = false;
      });
    };
    ...
    return (_ctx, _push, _parent, _attrs) => {
      ...
      _push(`<div${ssrRenderAttrs(_attrs)}><div id="btnnew"><a href="#" class="btn btn-info pull-right"><i class="fa fa-plus"></i></a><br><br></div>`);
      if (unref(showForm)) {
...
//# sourceMappingURL=index-D7NpioNS.mjs.map

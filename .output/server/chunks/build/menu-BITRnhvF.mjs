...
const useMenuStore = defineStore("menu", () => {
  ...
  const fetchMenuItems = async (page = 1) => {
    try {
      const response = await api.get(`/menus?page=${page}&limit=${pagination.value.limit}`);
  ...
  const fetchMenuTree = async (langId) => {
  ...
  const refreshLanguageCache = async (langId) => {
    try {
      const response = await api.get(`/menus`);
      if (response.success && response.data) {
        const tree = buildMenuTree(response.data.items || response.data);
  ...
  const clearCacheForLanguage = (langId) => {
  ...
  const findMenuItemInTree = (id, tree = menuTree.value) => {
...
//# sourceMappingURL=menu-BITRnhvF.mjs.map

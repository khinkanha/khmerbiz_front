...
import { ssrRenderAttrs, ssrInterpolate, ssrRenderAttr, ssrIncludeBooleanAttr, ssrLooseContain, ssrLooseEqual, ssrRenderList, ssrRenderStyle } from 'vue/server-renderer';
...
const _sfc_main = /* @__PURE__ */ defineComponent({
  ...
  setup(__props) {
    ...
    return (_ctx, _push, _parent, _attrs) => {
      _push(`<div${ssrRenderAttrs(_attrs)}><form><div class="row"><div class="col-md-6"><div class="form-group"><label>${ssrInterpolate(_ctx.$t("settings.languageTitle"))}</label><input type="text"${ssrRenderAttr("value", unref(langForm).lang_name)} class="form-control"${ssrRenderAttr("placeholder", _ctx.$t("settings.languageTitle"))}></div></div><div class="col-md-6"><div class="form-group"><label>${ssrInterpolate(_ctx.$t("settings.flagType"))}</label><select class="form-control"><option value=""${ssrIncludeBooleanAttr(Array.isArray(unref(langForm).flag) ? ssrLooseContain(unref(langForm).flag, "") : ssrLooseEqual(unref(langForm).flag, "")) ? " selected" : ""}>-- Select --</option><!--[-->`);
      ssrRenderList(flagOptions, (f) => {
        _push(`<option${ssrRenderAttr("value", f.value)}${ssrIncludeBooleanAttr(Array.isArray(unref(langForm).flag) ? ssrLooseContain(unref(langForm).flag, f.value) : ssrLooseEqual(unref(langForm).flag, f.value)) ? " selected" : ""}>${ssrInterpolate(f.label)}</option>`);
      });
      _push(`<!--]--></select></div></div></div><button type="submit" class="btn btn-danger"${ssrIncludeBooleanAttr(unref(adding)) ? " disabled" : ""}><i class="fa fa-floppy-o"></i> ${ssrInterpolate(_ctx.$t("settings.save"))}</button></form><div class="table-responsive" style="${ssrRenderStyle({ "margin-top": "15px" })}"><table class="table"><thead><tr><th></th><th></th><th></th><th></th><th></th></tr></thead><tbody><!--[-->`);
      ssrRenderList(unref(settingStore).languages, (lang, i) => {
        var _a;
        _push(`<tr><td>${ssrInterpolate(i + 1)}</td><td>${ssrInterpolate(lang.lang_name)}</td><td><img${ssrRenderAttr("src", `/flag/${((_a = flagOptions.find((f) => f.value === lang.flag)) == null ? void 0 : _a.icon) || "kh.svg"}`)} style="${ssrRenderStyle({ "width": "20px", "height": "14px", "border-radius": "2px" })}"></td><td>`);
        if (lang.is_default) {
          _push(`<i class="fa fa-star" style="${ssrRenderStyle({ "color": "#f0ad4e" })}"></i>`);
        } else {
          _push(`<a href="#"><i class="fa fa-star" style="${ssrRenderStyle({ "color": "#ccc" })}"></i></a>`);
        }
        _push(`</td><td><a href="#" class="text-danger">${ssrInterpolate(_ctx.$t("settings.removeImage"))}</a></td></tr>`);
      });
...
_sfc_main.setup = (props, ctx) => {
  ...
  (ssrContext.modules || (ssrContext.modules = /* @__PURE__ */ new Set())).add("pages/admin/settings/language.vue");
  return _sfc_setup ? _sfc_setup(props, ctx) : void 0;
};

export { _sfc_main as default };
//# sourceMappingURL=language-DY0cPFw9.mjs.map
